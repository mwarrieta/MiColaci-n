    ...
    return (
        <div className="space-y-8 pb-10">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-heading font-black tracking-tight text-white">Centro de Control</h1>
                    <p className="text-gray-400 font-medium text-sm mt-1">
                        Resumen pa&apos; hoy, {hoyDate.toLocaleDateString("es-CL", { weekday: "long", day: "numeric", month: "long" })}
                    </p>
                </div>
                <div className="flex items-center gap-3">
                    <Link
                        href="/admin/reportes"
                        className="flex items-center gap-2 bg-admin-surface border border-white/10 text-gray-300 hover:text-white hover:border-white/20 font-semibold px-4 py-2 rounded-xl transition-all text-sm"
                    >
            ...
            {/* Deuda Fiados */}
            {deudaFiados > 0 && (
                <div className="bg-red-500/10 rounded-3xl border border-red-500/20 p-5 sm:p-6 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <div className="w-12 h-12 bg-red-500/20 rounded-2xl flex items-center justify-center">
                            <BookOpen className="w-6 h-6 text-red-400" />
                        </div>
                        <div>
                            <p className="text-2xl font-black font-heading text-red-400">${deudaFiados.toLocaleString("es-CL")}</p>
                            <p className="text-sm font-bold text-red-300/70">Deuda Total en Libreta</p>
                        </div>
                    </div>
                    <Link href="/admin/pedidos" className="text-sm text-red-400 font-bold bg-red-500/10 px-4 py-2 rounded-xl border border-red-500/20 hover:bg-red-500/20 transition-colors">
                        Ver detalle
                    </Link>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-admin-surface rounded-3xl border border-white/5 overflow-hidden">
                    <div className="p-6 border-b border-white/5 flex items-center justify-between">
                        ...
                        <Link href="/admin/pedidos" className="text-sm text-brand-400 font-bold hover:text-brand-300 bg-brand-500/10 px-3 py-1.5 rounded-full transition-colors border border-brand-500/20">
                            Ver todos
                        </Link>
                    </div>
                    {!pedidosPendientes || pedidosPendientes.length === 0 ? (
                        <div className="p-12 text-center text-gray-400">
                            ...
                            <p className="text-sm font-bold">¡Todo al día, cariño! Sin pendientes.</p>
                        </div>
                    ) : (
                        <ul className="divide-y divide-white/5">
                            {pedidosPendientes.map((p) => {
                                ...
                                return (
                                    <li key={p.id} className="p-4 sm:p-5 hover:bg-white/5 transition-colors">
                                        <div className="flex items-start justify-between gap-2">
                                            <div className="min-w-0 flex-1">
                                                ...
                                                {p.hora_solicitada && (
                                                    <p className="text-xs text-brand-400 font-bold mt-1">⏰ Para las {p.hora_solicitada}</p>
                                                )}
                                                {p.direccion_entrega && (
                ...
                <div className="bg-admin-surface rounded-3xl border border-white/5 overflow-hidden">
                    ...
                    {pedidosHoy.length === 0 ? (
                        <div className="p-12 text-center text-gray-400">
                            ...
                            <p className="text-sm font-bold">Aún no hay pedidos hoy, cariño.</p>
                        </div>
                    ) : (
                        <ul className="divide-y divide-white/5 max-h-[350px] overflow-y-auto">
                            {pedidosHoy.map((p) => {
                                ...
                                return (
                                    <li key={p.id} className="p-4 sm:p-5 flex items-center justify-between hover:bg-white/5 transition-colors">
